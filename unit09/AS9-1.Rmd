...
3. 本公司會員，有飛行習慣(擁有所有群組裡最多的可用里程數)，但卻總是在其他聯盟    公司消費→重點發展顧客
4. 對我們營收沒貢獻，但從非飛行交易次數多但非飛行里程數少，推測可能是利用聯盟公    司頻繁飛短程的顧客→潛在短程顧客
5. 沒有飛行習慣→低價值顧客
...
#Ans：
未必見得，視情況而定。應依商業情境選擇分群，只要能針對各族群訂定出合適的策略，即是好的分群。 整體上除了2,5群，轉乘的里程數或是次數都是他們的共同特徵，統計上的分組沒有考慮到這個因素去分組是不好的，統計上的分組仍然存在許多組共同的特徵，而且這個共同特徵還是會影響的營收就更不好了，這塊上我們無法針對這個特徵去進行針對性行銷
