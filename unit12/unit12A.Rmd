...
#令綠色點為成功事件，即Y=1
#藍線表示logit=0、odd=1時，即Y=1(成功事件)發生的機率為0.5

#青線：
#藍線以上部分logit>0、odd>1，即Y=1(成功事件)發生的機率比Y=0(失敗事件)發生的機率高
#藍線以下部分logit<0、odd<1，即Y=1(成功事件)發生的機率比Y=0(失敗事件)發生的機率低

#左下至右上的線，分別代表Y=1(成功事件)的機率為0.1~0.9

<span style="font-size:24px"> `r "\U1F5FF"` : </span>
Given any point in the figure above, how can you tell its (predicted) probability approximately?<br>

#依據任一點其最靠近的線判斷發生Y=1(成功事件)的機率，以最右下角(一綠一紅)為例，兩點發生成功事件機率皆在0.5~0.6之間，實際結果為一個成功(綠點)一個失敗(紅點)
...
#logit：Odd = p/(1−p)為勝算，ln(Odd)=logit ，即對勝算取自然對數，為一線性函數
#logistic function：母體發生成功事件(Y=1)的機率，P=Exp(fx)/(1+Exp(fx))，非線性函數
#兩者之間可以互相轉換
