...
table(CPS$Citizenship == "Non-Citizen") %>% prop.table %>% round(3)
#0.942
```
...
#Americam Indian、Black、Multiracial、White
```
...
```{r}
abs(0.3-tapply(is.na(CPS$MetroAreaCode),CPS$State,mean)) %>% sort %>% head
#Winconsin
```
...
```{r}
tapply(CPS$Education=="No high school diploma",CPS$MetroArea,mean, na.rm=TRUE) %>% sort %>% head
#Iowa City, IA
```
