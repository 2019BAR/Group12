...
```{r}
GG = read.csv("data/mvtWeek1.csv", stringsAsFactors = F)
nrow(GG)
ncol(GG)
```
...
```{r}
sum(GG$LocationDescription=="ALLEY")
```
...
```{r}
head(GG$Date) #Month/Day/Year Hour：Minute  
```

```{r}
qq = as.POSIXct(GG$Date,format="%m/%d/%y")
par(cex=0.6)  #用來設定圖裡的參數
hist(qq,"year",las=2,freq = T,xlab = "")
qq
```

```{r}
table(format(qq,"%u"))   #每個禮拜幾的遺失數
table(format(qq,"%m"))   #每個月遺失次數
table(weekday=format(qq,"%u"),months=format(qq,"%m"))
```

```{r fig.height=2.5, fig.width=8}
table(format(qq,"%u"), format(qq,"%H")) %>% 
...
```{r}
TT = as.Date(strptime(GG$Date,"%m/%d/%y"))  
#strptime 把時間格式轉成年月日
median(TT)
```
...
```{r}
GG$Month <- months(TT)
which.min(table(GG$Month))
```

【2.4】 On which weekday did the most motor vehicle thefts occur?
```{r}
GG$weekday <- weekdays(TT)
which.max(table(GG$weekday))
#
```

【2.5】 Which month has the largest number of motor vehicle thefts for which an arrest was made?
```{r}
qq[GG$Arrest] %>% format('%m') %>% table %>% sort
B = subset(GG,Arrest == T)
H = as.Date(strptime(B$Date,"%m/%d/%y"))
B$Month = months(H)
table(B$Month) %>% sort
...
```{r}
table(H > as.Date("2007-01-01"))
#前半期逮捕數比後半期多
```

【3.3】 For what proportion of motor vehicle thefts in 2001 was an arrest made?
```{r}
table(GG$Arrest,format(qq,"%Y")) %>% prop.table(2) %>% round(3)
# Ans：0.104
```
...
```{r}
table(GG$LocationDescription) %>% sort %>% tail(6)
#
```

【4.2】 How many observations are in Top5?
```{r}
(top5 = names(table(GG$LocationDescription) %>% sort %>% tail(6))[-4])

sum(GG$LocationDescription %in% top5)  # 177510
```
```{r} 
top5s=names(table(GG$LocationDescription))
top5sins=sort(top5s)
top5sfin=tail(top5sins)[-4]

sum(GG$LocationDescription %in% top5sfin)  #為何不行
```

【4.3】 One of the locations has a much higher arrest rate than the other locations. Which is it? 
```{r}
TOP5 = subset(GG,LocationDescription %in% top5)
...
```{r}
qq[GG$LocationDescription == "GAS STATION"] %>% format("%w") %>% table %>% sort 
```

【4.5】 On which day of the week do the fewest motor vehicle thefts in residential driveways happen?
```{r}
qq[GG$LocationDescription == "DRIVEWAY - RESIDENTIAL"] %>% format("%w") %>% table %>% sort 
```
